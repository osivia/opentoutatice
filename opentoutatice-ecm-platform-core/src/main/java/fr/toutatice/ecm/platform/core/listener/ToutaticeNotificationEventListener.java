 ...
 * 
 */
public class ToutaticeNotificationEventListener extends
	...
	public boolean acceptEvent(Event event) {
		...
		if (ctx instanceof DocumentEventContext) {
			DocumentModel doc = ((DocumentEventContext) ctx)
					.getSourceDocument();
			if (doc != null) {
				boolean onLineWF = ToutaticeWorkflowHelper.isOnLineWorkflow(doc
						.getPath());
				boolean taskAssigned = TaskEventNames.WORKFLOW_TASK_ASSIGNED
						.equals(event.getName());
				if (onLineWF && taskAssigned) {
					return false;
				}
			}
		}
