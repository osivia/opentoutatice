(function() {
  ...
  function configureOperationParameters(op, params, query) {
    ...
    } else if (params.operationId == 'Directory.SuggestUserEntriesWithPermission') {
      temp.searchTerm = query.term;
      temp.searchType = params.userSuggestionSearchType;
      temp.permission = params.permission;
  ...
  function initToutaticeSelect2Widget(el) {
    ...
    if ($parentForm.hasClass("safeEditEnabled")) {
        ...
        jQuery(el).closest("form").registerPostRestoreCallBacks(function(data) {
          ...
          initToutaticeSelect2Widget(jQuery(el));
        });
      }
  }
  ...
  window.initToutaticeSelect2Widgets = function initToutaticeSelect2Widgets() {
