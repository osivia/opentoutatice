...
@Operation(id = ResizeImageOperation.ID, category = Constants.CAT_CONVERSION, label = "Image resizing operation",
        description = "Operation to resize the images")
public class ResizeImageOperation {
    ...
    private Object resize(Object image, int img_width, int img_heidth, boolean enlarge) throws NuxeoException {
        String imageName = "";
        Blob imageBlob = null;
        ...
        if (image instanceof Map) {
            ...
            log.debug("Paramètre d'entrée inconsistant '" + xpath_img_in
                    + "'. Doit désigner une donnée de type blob ou bien une donnée complexe du type Map<String, Blob>");
        }
        ...
        if (null != imageBlob) {
            ...
            if (roundToOne(scale) < 1 || (roundToOne(scale) > 1) && enlarge) {
                imageBlob = getImageService().resize(imageBlob, "jpg", (int) (width * scale), (int) (height * scale), info.getDepth());
    ...
    private ImagingService getImageService() throws NuxeoException {
        if (null == this.service) {
            try {
                service = Framework.getService(ImagingService.class);
            } catch (Exception e) {
                throw new NuxeoException("Failed to get ImagingService, error: " + e.getMessage());
            }
        }
