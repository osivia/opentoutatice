...
//        ToutaticeFunctions tf = new ToutaticeFunctions();
        String truncatedHtml = truncateHTML(html, 300);
    }
    
    public String truncateHTML(String html, int nbChars) {
        String truncatedHtml = html;
        System.out.println("HTML before: " + Jsoup.parse(html).text().length());

        if (html != null) {
            // Document
            Document document = Jsoup.parse(html);
            
            System.out.println("Body?: " + document.getElementsByTag("body"));
                ...
                if (elements != null) {
                    ...
                    for (int index = elements.size(); text.length() > nbChars && index > 0; index--) {
                        ...
                        if (lastTextElement.text() != null) {
                            ...
                            System.out.println("== inner: " + innerText.length());
                            System.out.println("== inner value: " + innerText);
                    ...
                    System.out.println("HTML after: " + Jsoup.parse(truncatedHtml).text().length());
                    System.out.println("HTML after: " + truncatedHtml);
                }
            }
