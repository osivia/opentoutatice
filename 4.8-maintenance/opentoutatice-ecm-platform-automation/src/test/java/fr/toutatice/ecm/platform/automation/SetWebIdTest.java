...
public class SetWebIdTest {
    ...
    private static final String TEST_PATH = "/toutatice/workspaces/wstestmigration/documents";

    public static void main(String[] args) throws Exception {

        try {
            HttpAutomationClient client = new HttpAutomationClient("http://mig44dch:8081/nuxeo/site/automation");
            ...
            try {
                ...
                otherCreatedDoc = (Document) session.newRequest(CreateDocument.ID).setInput(new PathRef(TEST_PATH)).set("type", "Note")
                        .set("properties", "dc:title=Note avec webid").setHeader(Constants.HEADER_NX_SCHEMAS, "toutatice").execute();
                ...
                otherCreatedDocForInc = (Document) session.newRequest(CreateDocument.ID).setInput(new PathRef(TEST_PATH)).set("type", "Note")
                        .set("properties", "dc:title=Note avec webid").setHeader(Constants.HEADER_NX_SCHEMAS, "toutatice").execute();
                ...
                orphanDraft = (Document) session.newRequest(CreateDocument.ID).setInput(new PathRef(TEST_PATH)).set("type", "Note")
                        .set("properties", "dc:title=Note avec webid").setHeader(Constants.HEADER_NX_SCHEMAS, "toutatice").execute();
                ...
                Document updatedDraftDoc = (Document) session.newRequest("Document.Update").setInput(orphanDraft)
                        .set("properties", "ttc:webid=draft_" + beforeDraftWebid).setHeader(Constants.HEADER_NX_SCHEMAS, "toutatice").execute();
                ...
                createdAfterOrphanDraft = (Document) session.newRequest(CreateDocument.ID).setInput(new PathRef(TEST_PATH)).set("type", "Note")
                        .set("properties", "dc:title=Note avec webid").setHeader(Constants.HEADER_NX_SCHEMAS, "toutatice").execute();
